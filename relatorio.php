...
$sql_relatorio_vendas = "SELECT vendas.produto_id, produtos.produto AS nome_produto, SUM(vendas.quantidade) AS total_vendido,
                         SUM(vendas.quantidade * produtos.preco) AS valor_total_vendido
                         FROM vendas
                         INNER JOIN produtos ON vendas.produto_id = produtos.id
                         GROUP BY vendas.produto_id
                         ORDER BY total_vendido DESC";
$resultado_relatorio_vendas = $conectar->query($sql_relatorio_vendas);
...
<main class="form-signin w-100 m-auto">
  <form action="" method="POST">
    ...
    <table class="table">
      ...
      <tbody>
      <?php while ($row = $resultado_relatorio_vendas->fetch_assoc()) { ?>
            <tr>
                <td><?php echo $row['produto_id']; ?></td>
                <td><?php echo $row['nome_produto']; ?></td>
                <td><?php echo $row['total_vendido']; ?></td>
                <td><?php echo 'R$ ' . number_format($row['valor_total_vendido'], 2, ',', '.'); ?></td>
            </tr>
        <?php } ?>
      </tbody>
    </table>
